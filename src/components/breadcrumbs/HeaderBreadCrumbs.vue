...
const type = computed(() => {
  const p = parts();
  return ((p.length > 0 && p[0]?.match(/news|sitemap|search/gi)?.length) ?? 0 > 0) ? p[0] : '';
});
const id = computed(() => {
  const p = parts();
  return p.length > 1 && type.value !== '' ? p[1] : '';
});
const date = computed(() => {
  const p = parts();
  return p.length > 2 && id.value !== '' ? p[2] : '';
});
...
<template>
  <div class="flex">
    <div v-if="type" class="flex items-center">
      ...
      <div class="text-caption text-capitalize q-ml-sm">{{ type }}</div>
    </div>
