...
const setup = () => {
  ...
  const Delete = async (botType: BotType, target: string) => {
    ...
    return await api
      ...
      .then(() => model.value.get(botType, []).filter((i) => i.target !== target))
  ...
  const Save = async (
    ...
    if (type === BotType.Sitemap && !IsValidDomain(target)) {
    ...
    return await api
      .put(`/bots?type=${type}`, { type, target, frequency, blackList, headless })
      .then((r: AxiosResponse<Bot>) => r.data)
      .then((bot: Bot) => {
        const bots = model.value.get(type, []);
        const idx = bots.findIndex((b) => b.target === target);
        if (idx > -1) {
          bots.splice(idx, 1);
        }
        bots.push(bot);
        model.value.set(type, bots);
        $notify.ok(null, `💾`, 'Saved');
        return bot;
      })
      .then(b => $router.push(`/${b.type}/${b.target}`))
      .catch($notify.err)
      .finally(Loading.hide);
  };
